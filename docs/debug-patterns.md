...
### 70. AI 코치 프롬프트가 사용자 제공 페이스를 우선하지 않음

**문제**: 사용자 제공 파일/표의 페이스가 있음에도 AI 코치가 참고 자료/일반 가이드를 우선해 다른 페이스를 제안함  
**원인**: 프롬프트에 사용자 제공 데이터 우선순위 및 sec/km 해석 규칙이 명시되지 않음  
**해결**: 시스템/플랜/워크아웃 프롬프트에 사용자 제공 데이터 우선 및 pace 단위 변환 지침 추가
...
- 컨텍스트의 pace 관련 숫자는 sec/km로 해석하고 M:SS/km로 변환합니다
- sec/km 값을 그대로 출력하지 않고 M:SS/km 형식만 사용합니다
```

**적용 위치**: `backend/app/core/ai_constants.py`
**날짜**: 2026-01-13
